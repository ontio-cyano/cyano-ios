    ...
    if ([super initWithFrame:frame]) {
        ...
        if (!ISEMPTY_S(text)) {
            CGSize textSize = [EasyShowUtils textWidthWithStirng:text
                                                        ...
                                                        maxWidth:[UIScreen mainScreen].bounds.size.width - 70];
...
//            self.textLabel.attributedText  = attStr;
            ...
            if (self.isShowedStatusBar||self.isShowedNavigation) {
                ...
                self.textLabel.frame = CGRectMake(12.5,self.easyS_height-textSize.height-11 ,textSize.width+15, textSize.height+2) ;
                if (textSize.width > [UIScreen mainScreen].bounds.size.width - 107.5) {
                    self.textLabel.textAlignment =NSTextAlignmentLeft  ;
                }else{
