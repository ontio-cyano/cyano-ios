...
//    [attStr addAttribute:NSParagraphStyleAttributeName value:style range: NSMakeRange(0, string.length)];
    ...
    CGSize size = [string boundingRectWithSize:CGSizeMake(maxWidth, SCREEN_HEIGHT_S)
                                    ...
                                    attributes:@{NSFontAttributeName:font}
                                       context:nil].size;
//    CGSize size = [attStr boundingRectWithSize:CGSizeMake(maxWidth, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading | NSStringDrawingTruncatesLastVisibleLine context:nil].size;
//    NSLog(@"hhhh%f",size.height);
    if (size.width < 60) {
        size.width = 60 ;
    }
    return size ;
