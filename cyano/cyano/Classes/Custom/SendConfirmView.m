...
- (UIButton *)confirmB {
    if (!_confirmB) {
        ...
        [_confirmB setTitle:@"Confirm" forState:UIControlStateNormal];
...
- (UIButton *)sureB {
    if (!_sureB) {
        ...
        [_sureB setTitle:@"Confirm" forState:UIControlStateNormal];
...
- (void)show {
    ...
    if (_isCapital||_isWalletBack){
        self.passwordV.center = CGPointMake(self.center.x, self.passwordV.center.y);
        self.titleL.text = @"Enter password";//Localized(@"EnterPassword");
